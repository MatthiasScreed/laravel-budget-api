...
class ProgressiveGamingService
{
    ...
    /**
     * Configuration des notifications selon le profil
     */
    private function getNotificationConfig(UserGamingProfile $profile): array
    {
        $level = $profile->effective_engagement_level;

        return [
            // Notifications de base (tous niveaux)
            'feedback_enabled' => true,
            'milestone_alerts' => true,

            // Notifications de récompenses (niveau 2+)
            'points_notifications' => $level >= UserGamingProfile::LEVEL_REWARDS,
            'level_up_celebrations' => $level >= UserGamingProfile::LEVEL_REWARDS,
            'achievement_popups' => $level >= UserGamingProfile::LEVEL_REWARDS,

            // Notifications sociales (niveau 3+)
            'streak_reminders' => $level >= UserGamingProfile::LEVEL_SOCIAL,
            'comparison_updates' => $level >= UserGamingProfile::LEVEL_SOCIAL,

            // Notifications gaming complet (niveau 4)
            'challenge_alerts' => $level >= UserGamingProfile::LEVEL_GAMING,
            'leaderboard_changes' => $level >= UserGamingProfile::LEVEL_GAMING,

            // Paramètres d'intensité
            'frequency' => match ($level) {
                1 => 'minimal',
                2 => 'moderate',
                3 => 'regular',
                4 => 'frequent',
            ...
            'sound_enabled' => $level >= UserGamingProfile::LEVEL_REWARDS,
            'vibration_enabled' => $level >= UserGamingProfile::LEVEL_SOCIAL,
        ];
    }
